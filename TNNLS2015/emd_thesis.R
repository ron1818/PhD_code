...
emd.rbfnn.predict<-emd.svr.predict<-emd.ann.predict<-array(NA,dim=c(nrow(tst$labels),ncol(tst$labels),nimf+1))
for (i in 1:(nimf+1)){ # for each IMF, +1 is for residue
	...
	emd.rbfnn.p.model<-my_cv_ann(tmp$trn,tmp$tst,method='rbf',n_h_range,is.ts=TRUE,cv.criteria='MSE')
	...
	emd.rbfnn.predict[,,i]<-emd.rbfnn.p.model$predict
...
# 52) take sum of IMFs discard the first IMF
emd.rbfnn.p.predict<-array(NA,dim=c(nrow(tst$labels),HORIZON))
for (h in 1:HORIZON)
  emd.rbfnn.p.predict[,h]<-apply(emd.rbfnn.predict[,h,-1],1,sum)

# 62) calculate error
emd.rbfnn.p.error<-my_forecasting_measure(emd.rbfnn.p.predict,tst$labels)

#########################################################
# 70) group IMF1-3 as HF 
# group IMF4-- as LF 
emd.LF.trn<-emd.HF.trn<-list(data=array(0,dim(emd.trn$data[[1]]),labels=array(0,dim(emd.trn$labels[[1]]))
emd.LF.tst<-emd.HF.tst<-list(data=array(0,dim(emd.tst$data[[1]]),labels=array(0,dim(emd.tst$labels[[1]]))
                                        
for (i in 1:3){
  emd.HF.trn$data<-emd.HF.trn$data+emd.trn$data[[i]]
  emd.HF.trn$labels<-emd.HF.trn$labels+emd.trn$labels[[i]]
  emd.HF.tst$data<-emd.HF.tst$data+emd.tst$data[[i]]
  emd.HF.tst$labels<-emd.HF.tst$labels+emd.tst$labels[[i]]
}

for (i in 4:(nimf+1)){
  emd.LF.trn$data<-emd.LF.trn$data+emd.trn$data[[i]]
  emd.LF.trn$labels<-emd.LF.trn$labels+emd.trn$labels[[i]]
  emd.LF.tst$data<-emd.LF.tst$data+emd.tstdata[[i]]
  emd.LF.tst$labels<-emd.LF.tst$labels+emd.tst$labels[[i]]
}

# 80) predict HF and LF
svr.HF.model<-my_cv_svm(emd.HF.trn,emd.HF.tst,kernel='radial',is.ts=TRUE,cv.criteria='MSE')
svr.LF.model<-my_cv_svm(emd.LF.trn,emd.LF.tst,kernel='polynomial',is.ts=TRUE,cv.criteria='MSE')

emd.HL.predict<-svr.HF.model$predict+svr.LF.model$predict
